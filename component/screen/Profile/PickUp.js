...
import { Text, StyleSheet, View, FlatList, Image, TouchableOpacity, TextInput, ScrollView ,CheckBox,Alert, AsyncStorage} from 'react-native'
...
class MyListItem extends Component {
    ...
    render() {
      ...
      return (
        <TouchableOpacity style={{paddingVertical:7, paddingRight:12,  margin:5, flexWrap:'wrap'}} onPress={this._onPress}>
          <View style={{flexDirection:'row'}}>
            <View style={{ height:13, width:13, borderWidth:1, borderColor:'#9E9E9E',backgroundColor:textColor,justifyContent:"center", alignItems:'center', marginRight:10, marginTop:3}}></View>
            <Text style={{ color: '#194F9E' ,fontFamily: 'Montserrat-SemiBold',fontSize:15}}>
              {this.props.title}
            </Text>
          </View>
        </TouchableOpacity>
...
export default class PickUp extends Component {
    ...
    _getSeatPan = () => {
        axios.get(`${this.state.settings.base_url}Api/findBookingInformation?route=${this.state.data.route}&trip_id_no=${this.state.data.trip_id_no}&booking_date=${this.state.day}&type=${this.state.fleetType}`)
            .then(res => {
                this.setState({
                    ...this.state,
    ...
    _countNavi = () => {
        ...
        let total_Seat = (this.state.adult*1) + (this.state.child*1) + (this.state.special*1)
        ...
        if(this.state.pickupData == '' || this.state.dropData == ''){
            ...
            if(this.state.adult == '' && this.state.child == '' && this.state.special == ''){
                ...
                if(this.state.ava > total_Seat){
                    this.props.navigation.navigate('seatplan', { pickup : this.state, settings : this.state.settings})
                }else{
                ...
                if(this.state.seatdataInfo.offers == null){
                    ...
                    return <View style={{marginBottom:10}}>
                            <View style={{marginBottom:8}}>
                                <Text style={styles.locationDet}>{this.state.settings.offer_code}</Text>
                            </View>
    ...
    _getFacilitiesData = () => {
        if(this.state.seatdataInfo == ''){
            ...
            if(this.state.seatdataInfo.facilities == '' || this.state.seatdataInfo.facilities == null){
                return <View>
                    <Text style={{marginLeft:7}}>{this.state.settings.no_facilities_available}</Text>
                </View>
            }else{
                return <FlatList
            ...
            <View style={{flex: 1}}>
                ...
                <Container style={{backgroundColor: '#F9F9F9'}}>
                    ...
                    <View style={[styles.searchtour,{flexDirection:'row'}]}>
                        ...
                        <View style={{flex:2}}>
                            <Button transparent onPress={()=> this.props.navigation.navigate('journeydetails')}>
                                <Icon style={{color:'#fff'}} name='arrow-back' />
                            </Button>
                          </View>
                          <View style={{flex:14}}>
                            <Text style={[styles.searchtourtext]}>{this.state.settings.pickup_and_drop}</Text>
                        </View>
                    ...
                    <ScrollView>
                        <View style={styles.AllDataVal}>
                            ...
                            {/* <View style={{marginBottom:10}}>
                                ...
                                <View style={{borderWidth:2, borderColor:'#fff', backgroundColor:'#fff', paddingHorizontal:10}}>
                                        <Picker
                                            mode='dropdown'
                            ...
                            <View style={{flexDirection:'row',marginBottom:10, }}>
                                ...
                                <View style={{flex:1}}>
                                    <View style={{marginBottom:8}}>
                                        <Text style={styles.locationDet}>{this.state.settings.adult}</Text>
                                    </View>
                                ...
                                <View style={{flex:1}}>
                                    <View style={{marginBottom:8}}>
                                        <Text style={styles.locationDet}>{this.state.settings.children}</Text>
                                    </View>
                                ...
                                <View style={{flex:1}}>
                                    <View style={{marginBottom:8}}>
                                        <Text style={styles.locationDet}>{this.state.settings.special}</Text>
                                    </View>
                            ...
                            <View style={{marginBottom:10}}>
                                <View style={{marginBottom:8}}>
                                    <Text style={styles.locationDet}>{this.state.settings.facilities}</Text>
                                </View>
