...
import { Text, StyleSheet, View , Image , TouchableOpacity,Alert, AsyncStorage } from 'react-native'
...
export default class Profile extends Component {
    ...
    _pointAllData = () => {
        setTimeout(()=>{
            axios.get(`${this.state.settings.base_url}Api/location`)
            .then(res => {
                // console.warn(res)
    ...
    _searchJourneyTour = () =>{
        ...
        if(this.state.startPointVal == '' || this.state.endPointVal == '' || this.state.fleetsData =='' || this.state.chosenDate== ''){
            Alert.alert(
    ...
    getfleetType = () => {
        ...
        axios.get(`${this.state.settings.base_url}Api/fleet_list`)
            .then(res => {
    ...
    _getProDataAll = () =>{
        if(this.state.settings == '' || this.state.userDa ==''){
            ...
            <Container>
                ...
                <Content style={{backgroundColor:'#F9F9F9'}}>
                    <View style={[styles.searchtour]}>
                        <Text style={[styles.searchtourtext]}>{this.state.settings.search_tour.toUpperCase()}</Text>
                    </View>

                    <View style={{flex:1}}>
                    <View style={{width:'96%', marginLeft:'2%', marginVertical:15}}>

                        <View style={styles.locksearchDet}>
                            <View style={{flexDirection:'row'}}>
                                {locationIcon}
                                <Text style={styles.locationDet}>{this.state.settings.location_details}</Text>
                            </View>

                            <View style={{flexDirection:'row', marginTop:15}}>

                                <View style={{flex:4}}>
                                    <Text style={styles.startpoint}>{this.state.settings.start_point}</Text>
                                    <Picker
                                        mode='dropdown'
                                        selectedValue={this.state.startPointVal}
                                        onValueChange={this.onstartValueChange.bind(this)}>
                                        <Item key='k' label={this.state.settings.select_start_point} value='' />
                                        { this._getStatData() }
                                    </Picker>
                                </View>
                                ...
                                <View style={{flex:4,}}>
                                    <Text style={styles.startpoint}>{this.state.settings.end_point}</Text>
                                    <Picker
                                        mode='dropdown'
                                        selectedValue={this.state.endPointVal}
                                        onValueChange={this.onendValueChange.bind(this)}>
                                        <Item key='k' label={this.state.settings.select_end_point} value='' />
                                        { this._getStatData() }
                                    </Picker>
                                </View>
                        ...
                        <View style={styles.locksearchDet}>
                            ...
                            <View style={{flexDirection:'row', marginBottom:8}}>
                                {timeIcon}
                                <Text style={styles.locationDet}>{this.state.settings.journey_date}</Text>
                            </View>
                            ...
                            <View style={{flexDirection:'row'}}>
                                <View style={{flex:8}}>
                                    <DatePicker
                                        ...
                                        animationType={"slide"}
                                        androidMode={"default"}
                                        placeHolderText= {this.state.settings.select_journey_date}
                                        textStyle={{ color: "#000" }}
                                ...
                                <View style={{flex:1}}>
                                <Image
                                    style={{width:25, height:25, resizeMode:'contain', marginTop:8}}
                        ...
                        <View style={styles.locksearchDet}>
                            ...
                            <View style={{flexDirection:'row', marginBottom:8}}>
                                {/* {timeIcon} */}
                                <Text style={styles.locationDet}>{this.state.settings.fleet_type}</Text>
                            </View>
                            ...
                            <View style={{flexDirection:'row'}}>
                                    <Picker
                                        ...
                                        <Item key='k' label={this.state.settings.select_fleet_type} value='' />
                                        { this._getfleetsData() }
                                    </Picker>
                            </View>
