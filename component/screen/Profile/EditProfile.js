...
import { Text, StyleSheet, View, Image, TouchableOpacity, ScrollView, Alert, TextInput,AsyncStorage } from 'react-native'
import { Header,Body , Right, Icon, Thumbnail, Button} from 'native-base'
...
export default class EditProfile extends Component {
    ...
    _saveProfile = () =>{
        ...
        const old_one = md5(this.state.old_password)
        ...
        console.log(this.state.userDa.password)
         ...
         if(this.state.old_password == '' || this.state.fname == '' || this.state.lname == ''){
            Alert.alert(
            ...
            if(this.state.userDa.password == old_one){
                ...
                if(this.state.password == ''){
                    ...
                    axios.get(`${this.state.settings.base_url}Api/user_update?id_no=${this.state.userDa.id_no}&firstname=${this.state.fname}&lastname=${this.state.lname}&email=${this.state.email}&password=${this.state.old_password}&phone=${this.state.ephone}&nid&address=${this.state.eaddress}`)
                    .then( res  => {
                    ...
                    axios.get(`${this.state.settings.base_url}Api/user_update?id_no=${this.state.userDa.id_no}&firstname=${this.state.fname}&lastname=${this.state.lname}&email=${this.state.email}&password=${this.state.password}&phone=${this.state.ephone}&nid&address=${this.state.eaddress}`)
                    .then( res  => {
                        if(res.data.response.status == 'ok' ){
    ...
    _viewPro = () =>{
            return <ScrollView>
                    <View style={[styles.searchtour,{flexDirection:'row'}]}>
                        ...
                        <View style={{flex:14}}>
                            <Text style={[styles.searchtourtext]}>{this.state.settings.edit_profile}</Text>
                        </View>
    ...
    render() {
        if(this.state.userDa == '' || this.state.settings == '' || this.state.logo == ''){
